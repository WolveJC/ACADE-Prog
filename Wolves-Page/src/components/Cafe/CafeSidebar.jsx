...
const CafeSidebar = () => {
        ...
        return (
            <ul className="space-y-3">
                {staticNutritionData.map((item, index) => {
                    ...
                    if (displayValue === null && item.name !== 'Calorías' && item.name !== 'Proteínas' && item.name !== 'Carbohidratos' && item.name !== 'Grasas Totales') {
                    ...
                    return (
                        <li key={index} className="flex items-center space-x-2 text-white/90">
                            <IconComponent className={`${item.iconColor} text-lg md:text-xl flex-shrink-0`} />
    ...
    return (
        <div 
            ...
            "
        >
            <h3 className="text-xl font-serif font-bold text-pan-tostado mb-4 text-center">Nutrición</h3>
            {renderContent()}
        </div>
    );
};
